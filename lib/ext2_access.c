...
__u32 get_block_size(void * fs) {
    ...
    int shift = super_block->s_log_block_size;
    __u32 result = SUPERBLOCK_SIZE << shift;
    return result;
}
...
void * get_block(void * fs, __u32 block_num) {
    void* result = fs + get_block_size(fs) * block_num;
    return result;
}
...
struct ext2_group_desc * get_block_group(void * fs, __u32 block_group_num) {
    return (struct ext2_group_desc*) (fs + SUPERBLOCK_SIZE + SUPERBLOCK_OFFSET);
}
...
struct ext2_inode * get_inode(void * fs, __u32 inode_num) {
    struct ext2_group_desc *group_desc = get_block_group(fs, 0);
    void *block = get_block(fs, group_desc->bg_inode_table);
    return (struct ext2_inode*)(block + (inode_num - 1) * sizeof(struct ext2_inode));
}
...
__u32 get_inode_from_dir(void * fs, struct ext2_inode * dir, 
        char * name) {
    struct ext2_dir_entry_2* block = (struct ext2_dir_entry_2*) get_block(fs, dir->i_block[0]);
    while(block->inode != 0) {
        int i;
        int flag = 0;
        for(i = 0; i < block->name_len; i++) {
        ...
        if(flag == 0)
            return block->inode;
        block = (struct ext2_dir_entry_2*)((void*)block + block->rec_len);
    }
    return 0;
}
...
__u32 get_inode_by_path(void * fs, char * path) {
    ...
    char** split_path_vec = split_path(path);
    struct ext2_dir_entry_2* cur_dir = (struct ext2_dir_entry_2*)get_block(fs, dir->i_block[0]);
    int i = 0;
    while(cur_dir->file_type == EXT2_FT_DIR) {
        result = get_inode_from_dir(fs, dir, split_path_vec[i]);
        if(result == 0) return 0;
        dir = get_inode(fs, result);
        cur_dir = (struct ext2_dir_entry_2*)get_block(fs, dir->i_block[0]);
        i++;
    }
    return result; 
}
