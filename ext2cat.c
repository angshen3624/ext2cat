...
int main(int argc, char ** argv) {
    ...
    // continue for i > EXT2_NDIR_BLOCKS
    void* block_indir = get_block(fs, target_ino->i_block[EXT2_NDIR_BLOCKS]);
    __u32* block_cur = (__u32*)block_indir;
    // loop to write to buf until no block to get
    while (bytes_read < size) {
        bytes_left = size - bytes_read;
        __u32 bytes_to_read = bytes_left > block_size ? block_size : bytes_left;
        void* block = get_block(fs, *block_cur);
        memcpy(buf + bytes_read, block, bytes_to_read);
        bytes_read += bytes_to_read;
        block_cur++;
    }
